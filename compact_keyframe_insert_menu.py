...
def use_visual_update(self,context):
    ...
    key_set.use_insertkey_override_visual=key_set.use_insertkey_visual=context.scene.better_keyframe_insert_menu.use_visual
...
class KeySettings(bpy.types.PropertyGroup):
    ...
    use_local_rotation:bpy.props.BoolProperty(name="Local rotation",default=True)
...
class CompactKeyframeInsertMenu(bpy.types.Operator):
    ...
    def draw(self,context):
        ...
        if self.active_key_set==BUILTIN_KSI_transform.bl_label:
            ...
            row=self.layout.row()
            row.prop(context.scene.better_keyframe_insert_menu,"use_location")
            row.prop(context.scene.better_keyframe_insert_menu,"use_rotation")
            row.prop(context.scene.better_keyframe_insert_menu,"use_scale")
            # self.layout.separator()

            self.layout.prop(context.scene.better_keyframe_insert_menu,"use_visual")

            if context.mode!='POSE':
                self.layout.prop(context.scene.better_keyframe_insert_menu,"use_delta")

            if self.gv is not None:
                self.layout.label(text="FreeIK")
                self.layout.prop(context.scene.better_keyframe_insert_menu,"use_linked")
                self.layout.prop(context.scene.better_keyframe_insert_menu,"use_local_rotation")
...
class BUILTIN_KSI_transform(bpy.types.KeyingSetInfo):
    ...
    def get_items(self,context):
        ...
        if gv is not None and context.scene.better_keyframe_insert_menu.use_linked:
    ...
    def poll(self,context):
        ...
        settings=context.scene.better_keyframe_insert_menu
    ...
    def generate(self,context,ks,item):
        ...
        if context.scene.better_keyframe_insert_menu.use_delta and context.mode=='OBJECT':
        ...
        if context.scene.better_keyframe_insert_menu.use_location:
            ks.paths.add(fcurve_holder,base_path+location_name)
        if context.scene.better_keyframe_insert_menu.use_rotation:
            ks.paths.add(fcurve_holder,base_path+rotation_name)
        if context.scene.better_keyframe_insert_menu.use_scale:
            ks.paths.add(fcurve_holder,base_path+scale_name)

        if  hasattr(bpy.types.Scene,"free_ik_gv"):gv=bpy.types.Scene.free_ik_gv
        else:gv=None

        if gv and context.scene.better_keyframe_insert_menu.use_local_rotation:
...
def register():
    ...
    bpy.types.Scene.better_keyframe_insert_menu=bpy.props.PointerProperty(type=KeySettings)
